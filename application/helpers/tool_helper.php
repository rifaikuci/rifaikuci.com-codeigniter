...
function turKullaniciResim($id)
{
    $ci =& get_instance();
    $sonuc = $ci->db->select('id,resim')->from('turKullanici')
        ->where('id', $id)->get()->row();
    $resim = $sonuc->resim;
    $silResimDizi = explode("/", $resim);
    $silResim = $silResimDizi[count($silResimDizi) - 4] . "/" . $silResimDizi[count($silResimDizi) - 3] . "/" .
...
{
    $ci =& get_instance();
    $sonuc = $ci->db->select('id')->from('turler')->where('tur', "Bitki")
        ->count_all_results();
    return $sonuc;
}

function hayvanSayisi()
{
    $ci =& get_instance();
    $sonuc = $ci->db->select('id')->from('turler')->where('tur', "Kuş")
        ->count_all_results();
    return $sonuc;
}

function turKesfetResim($id)
{
    $ci =& get_instance();
    $sonuc = $ci->db->select('id,turResim')->from('kesfet')
        ->where('id', $id)->get()->row();
    $resim = $sonuc->turResim;
    $silResimDizi = explode("/", $resim);
    $silResim = $silResimDizi[count($silResimDizi) - 4] . "/" . $silResimDizi[count($silResimDizi) - 3] . "/" .
        $silResimDizi[count($silResimDizi) - 2] . "/" .
        $silResimDizi[count($silResimDizi) - 1];
...
function aktifKesfet()
{
    $ci =& get_instance();
    $sonuc = $ci->db->select('id')->from('kesfet')->where('durum', 1)
        ->count_all_results();
    return $sonuc;
}

function pasifKesfet()
{
    $ci =& get_instance();
    $sonuc = $ci->db->select('id')->from('kesfet')->where('durum', 0)
        ->count_all_results();
    return $sonuc;
}
